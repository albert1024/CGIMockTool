...
namespace google { namespace protobuf { namespace compiler {namespace objectivec {
  namespace {
    void GenerateMacro(io::Printer* printer) {
      printer->Print(
        "#ifndef __has_attribute\n"
	"#define __has_attribute(x) 0  // Compatibility with non-clang compilers\n"
	"#endif\n\n"
	"#if __has_attribute(objc_method_family)\n"
	"  #define PB_OBJC_METHOD_FAMILY_NONE __attribute__((objc_method_family(none)))\n"
	"#else\n"
	"  #define PB_OBJC_METHOD_FAMILY_NONE\n"
	"#endif\n");  
    }
  }
  ...
  FileGenerator::~FileGenerator() {
    ...
    void FileGenerator::GenerateMockCase(io::Printer *printer) {
        printer->Printer::Print("var response = {");
    }
  ...
  void FileGenerator::GenerateHeader(io::Printer* printer) {
    ...
    if (file_->name() != "google/protobuf/descriptor.proto") {
      printer->Print("#import <MMCommon/ProtocolBuffers.h>\n\n");
    }

    if (file_->dependency_count() > 0) {
      for (int i = 0; i < file_->dependency_count(); i++) {
        if (FilePath(file_->dependency(i)) == "objectivec-descriptor") {
          continue;
        }
        printer->Print(
          "#import \"$header$.pb.h\"\n",
